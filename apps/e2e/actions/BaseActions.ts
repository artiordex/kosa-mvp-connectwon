...
export class BaseActions {
  ...
  // 특정 URL 패턴까지 대기
  async waitForUrl(partial: string | RegExp, timeout = 10_000): Promise<void> {
    await this.page.waitForURL(partial as any, { timeout });
  }

  // ----- Basic waits -----
  // 요소가 보일 때까지 대기
  async waitVisible(target: string | Locator, timeout = 10_000): Promise<Locator> {
  ...
  // 요소가 숨겨질 때까지 대기
  async waitHidden(target: string | Locator, timeout = 10_000): Promise<Locator> {
    const loc = this.locator(target);
  ...
  async waitEnabled(target: string | Locator, timeout = 10_000): Promise<Locator> {
    const loc = this.locator(target);
  ...
  async expectText(target: string | Locator, expected: string | RegExp): Promise<void> {
    await expect(this.locator(target)).toHaveText(expected as any);
  }

  // 텍스트 포함 검사
  async expectContainsText(target: string | Locator, expected: string | RegExp): Promise<void> {
    await expect(this.locator(target)).toContainText(expected as any);
  }
