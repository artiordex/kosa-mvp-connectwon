...
export default function Header() {
  ...
  const navItems = [
    ...
    { href: '/venues', label: '공간 및 디바이스', desc: '최첨단 시설과 장비' },
            ...
            <div className="flex items-center space-x-4">
              ...
              <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-gray-700 cursor-pointer relative z-60">
                <i
                  ...
                  } w-10 h-10 flex items-center justify-center text-2xl transition-all duration-300`}
                ></i>
              </button>
            </div>
          </div>
                  ...
                  {navItems.map(item => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="text-2xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors duration-300">{item.label}</h4>
                          {item.desc && <p className="text-gray-600 mt-1 text-sm">{item.desc}</p>}
                        </div>
