...
function fmtDateISO(d: Date = new Date()): string {
...
function relativeTimeFromISO(iso: string): string {
  try {
    ...
    const now = new Date();
    const diff = Math.floor((now.getTime() - target.getTime()) / (1000 * 60 * 60 * 24)); // in days
    if (diff === 0) return '오늘';
    if (diff === 1) return '어제';
...
function maskEmail(email?: string): string {
  ...
  const s = email.trim();
  const at = s.indexOf('@');
  // '@'가 없거나 맨 앞/맨 뒤면 마스킹 불가 → 원문 반환
  if (at <= 0 || at === s.length - 1) return s;

  const id = s.slice(0, at);
  const domain = s.slice(at + 1);

  // 아이디가 1글자면 그대로, 2글자 이상이면 앞 2글자만 보이고 나머지는 ***
  const visible = id.slice(0, 2);
  const maskedTailLen = Math.min(3, Math.max(0, id.length - visible.length));
  const masked = `${visible}${'*'.repeat(maskedTailLen)}`;
  return `${masked}@${domain}`;
}

function maskPhone(phone?: string): string {
  if (!phone) return '';
  // 하이픈 포맷일 때만 마스킹 (미매칭이면 원문 그대로)
  return phone.replace(
    /(\d{3})-(\d{4})-(\d{4})/,
    (_m, a: string, _b: string, c: string) => `${a}-****-${c}`
  );
}
...
function initials(name?: string): string {
  ...
  // string | undefined 도 안전하게 받도록 시그니처 수정
  const firstGrapheme = (s?: string): string =>
    (s ? Array.from(s)[0] : undefined)?.toUpperCase() ?? '';
...
function randomPastel(seed: number) {
  // deterministic small palette by seed
  const colors = [
...
export default function ProgramInquirySection() {
  ...
  // 폼 데이터 + 초안 자동 저장
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', password: '', title: '' });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setFormData({ name: '', email: '', phone: '', password: '', title: '' });
  ...
  const handlePasswordConfirm = () => {
    ...
    if (actionType === 'delete') {
      ...
      // 선택되어 있던 상세 보기 해제
      if (selectedInquiryId === targetInquiry.id) {
  ...
  const handleRowClick = (id: number) => {
    setSelectedInquiryId(selectedInquiryId === id ? null : id);
  };

  const handleDeleteDirect = (id: number) => {
  ...
  const Avatar = ({ name, seed }: { name: string; seed: number }) => (
    <div
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg w-[360px] p-6 animate-fade-up">
            ...
            <p className="text-xs text-gray-500 mb-4 text-center">
              수정 또는 삭제를 진행하려면 4자리 비밀번호를 입력하세요.
            </p>
            <input
              type="password"
              value={passwordInput}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="titleAsc">제목 오름차</option>
              <option value="titleDesc">제목 내림차</option>
            </select>
          </div>
        ...
        <form onSubmit={handleSubmit} className="border rounded-lg bg-white p-6 mb-10 shadow-sm space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center gap-4">
              <label className="w-24 text-sm font-semibold text-gray-700">이름</label>
              <input
                type="text"
                name="name"
                value={formData.name}
            ...
            <div className="flex items-center gap-4">
              <label className="w-24 text-sm font-semibold text-gray-700">휴대폰</label>
              <input
                type="tel"
                name="phone"
            ...
            <div className="flex items-center gap-4">
              <label className="w-24 text-sm font-semibold text-gray-700">비밀번호</label>
              <input
                type="password"
                name="password"
          ...
          <div className="flex items-center gap-4">
            <label className="w-24 text-sm font-semibold text-gray-700">제목</label>
            <input
              type="text"
              name="title"
                  ...
                  {currentPageItems.map((inq, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={(e) => {
                          if ((e.target as HTMLElement).tagName !== 'BUTTON' && (e.target as HTMLElement).tagName !== 'I') {
                      ...
                      >
                        ...
                        <td className="text-center">
                          <div className="flex items-center justify-center gap-2">
                            <Avatar name={inq.name || '익명'} seed={inq.id} />
                            <div className="text-sm leading-tight">
                              <div className="font-medium">{inq.name || '익명'}</div>
                              <div className="text-[11px] text-gray-500">{maskEmail(inq.email)}</div>
                            </div>
                          </div>
                        </td>
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600 flex items-center gap-2">
                      <span className="inline-flex items-center gap-2">
                        <Avatar name={selectedPost.name || '익명'} seed={selectedPost.id} />
                      ...
                      {selectedPost.phone && (
                        <>
                          <span className="text-gray-300">|</span>
                          <span className="text-xs text-gray-500">{maskPhone(selectedPost.phone)}</span>
                        </>
                      )}
                    </div>
                  </div>
...
function AnswerSection({
  ...
  const addReplyNested = (replies: Reply[], parentId: number, newReply: Reply): Reply[] => {
    return replies.map((reply) => {
      if (reply.id === parentId) {
        ...
        return { ...reply, replies: addReplyNested(reply.replies, parentId, newReply) };
      }
      return reply;
    });
  };
  ...
  const handleAddNested = (parentId: number) => {
    ...
    setInquiries((prev) =>
      prev.map((p) => (p.id === post.id ? { ...p, answers: addReplyNested(p.answers, parentId, newReply) } : p))
    );
              ...
              {replyingTo === answer.id && (
                <div className="mt-2">
                  ...
                  <div className="flex justify-end space-x-2 mt-1">
                    ...
                    <button
                      onClick={() => handleAddNested(answer.id)}
...
function ReplySection({
  ...
  const handleAddNested = (parentId: number) => {
    ...
    setInquiries((prev) =>
      prev.map((p) => (p.id === post.id ? { ...p, replies: addReplyNested(p.replies, parentId, newReply) } : p))
    );
    toast.success('답글이 등록되었습니다!');
    setNestedText('');
    setReplyingTo(null);
          ...
          {replyingTo === reply.id && (
            <div className="mt-2">
              ...
              <div className="flex justify-end space-x-2 mt-1">
                ...
                <button
                  onClick={() => handleAddNested(reply.id)}
                  className="text-xs bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700"
                >
                  등록
                </button>
